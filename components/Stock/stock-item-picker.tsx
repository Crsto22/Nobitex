    ...
    <div className="space-y-4">
      <div className="relative">
        ...
        <input
          ...
          placeholder={sucursalId ? "Buscar producto, SKU o codigo" : "Selecciona una ubicacion"}
          disabled={!sucursalId}
      ...
      {sucursalId && search.trim() ? (
        <div className="max-h-56 space-y-2 overflow-y-auto rounded-[14px] bg-[var(--color-background)] p-2">
          {loading ? (
            <p className="p-4 text-center text-sm text-[var(--color-muted-foreground)]">Buscando...</p>
          ) : options.length ? (
            options.map((option) => (
              <button
              ...
              >
                <span className="min-w-0">
                  <span className="block truncate text-sm font-circular-bold text-[var(--color-text)]">{option.label}</span>
                  <span className="text-xs text-[var(--color-muted-foreground)]">{option.sku || "Sin SKU"} · Stock {option.available}</span>
                </span>
                <PlusIcon size={18} weight="bold" className="text-[var(--color-primary)]" />
              </button>
            ))
          ) : (
            <p className="p-4 text-center text-sm text-[var(--color-muted-foreground)]">No se encontraron productos</p>
          )}
        </div>
      ) : null}
      ...
      <div className="space-y-2">
        {items.map((item) => (
          ...
          >
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-circular-bold text-[var(--color-text)]">{item.label}</p>
              <p className="text-xs text-[var(--color-muted-foreground)]">{item.sku || "Sin SKU"} · Disponible {item.available}</p>
            </div>
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => setQuantity(item.productoVarianteId, item.cantidad - 1)} className="flex h-9 w-9 items-center justify-center rounded-xl bg-[var(--color-input-bg)]" aria-label="Disminuir cantidad">
              ...
              />
              <button type="button" onClick={() => setQuantity(item.productoVarianteId, item.cantidad + 1)} disabled={enforceAvailable && item.cantidad >= item.available} className="flex h-9 w-9 items-center justify-center rounded-xl bg-[var(--color-input-bg)] disabled:opacity-40" aria-label="Aumentar cantidad">
                <PlusIcon size={15} weight="bold" />
              </button>
              <button
                type="button"
                onClick={() => onChange(items.filter((entry) => entry.productoVarianteId !== item.productoVarianteId))}
                className={cn("flex h-9 w-9 items-center justify-center rounded-xl bg-[#ef4444]/10 text-[#ef4444]")}
        ...
        {!items.length ? (
          <div className="rounded-[14px] bg-[var(--color-background)] px-4 py-8 text-center text-sm text-[var(--color-muted-foreground)]">Agrega al menos un producto</div>
        ) : null}
      </div>
    </div>
  );
