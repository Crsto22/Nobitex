...
type CategoryForm = typeof defaultForm;
...
export default function CatalogoCategoriasPage() {
  ...
  const refreshCategories = async (targetPage = currentPage) => {
  ...
  const toggleActive = async (category: Category) => {
    ...
    try {
      const updatedCategory = await categoriesApi.update(category.id, {
        activo: !category.activo,
      });
      await refreshCategories();
      showToast({
        title: updatedCategory.activo
          ? "Categoria activada"
          : "Categoria inactivada",
        description: updatedCategory.nombre,
        variant: "success",
      });
    } catch (error) {
      const message =
  ...
  return (
    <DashboardShell headerTitle="Categorias">
      <div className="content-scrollbar flex h-[calc(100dvh-4rem)] min-h-0 flex-col gap-3 overflow-y-auto bg-[var(--color-background)] p-3 transition-colors duration-200 sm:gap-4 sm:p-4 lg:px-6">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4">
          <MetricCard
            icon={<FolderSimpleIcon size={22} weight="fill" />}
        ...
        <div className="sticky -top-4 z-30 -mx-4 flex flex-col gap-3 bg-white px-4 py-2 sm:flex-row sm:items-center lg:-mx-6 lg:px-6 dark:bg-[var(--color-background)]">
          <label className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Buscar por categoria, descripcion o codigo..."
              value={searchTerm}
              onChange={(event) => {
        ...
        {isLoading ? (
          ...
          <div className="grid gap-3 pb-2">
            {categories.map((category) => {
              ...
              return (
                <div
                  key={category.id}
                  className="grid grid-cols-2 gap-x-3 gap-y-2 rounded-[14px] bg-[var(--color-card)] p-3 shadow-[0_2px_10px_rgba(21,25,34,0.12)] transition-colors hover:shadow-[0_4px_16px_rgba(21,25,34,0.16)] sm:p-4 md:grid-cols-[minmax(220px,1.3fr)_minmax(220px,1.2fr)_minmax(130px,0.7fr)_minmax(110px,0.6fr)_40px] md:items-center md:gap-5 md:gap-y-0"
                >
                    ...
                    {openMenuId === category.id && (
                      <div className="absolute right-0 top-full z-20 mt-2 w-44 rounded-xl bg-[var(--color-card)] p-1 shadow-lg ring-1 ring-[var(--color-border)]">
                        ...
                        >
                          ...
                          {category.activo ? "Inactivar" : "Activar"}
                        </button>
                        <button
                          type="button"
          ...
          <div className="flex min-h-[280px] flex-col items-center justify-center rounded-[14px] bg-[var(--color-card)] p-8 text-center shadow-[0_2px_10px_rgba(21,25,34,0.08)]">
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[var(--color-primary)]/10 text-[var(--color-primary)]">
              <FolderSimpleIcon size={26} weight="fill" />
            </div>
            ...
            <p className="mt-2 max-w-md text-sm font-medium text-[var(--color-muted-foreground)]">
              Crea categorias como polos, zapatillas, accesorios, uniformes o
              ropa deportiva para organizar tus productos.
            </p>
            <Button
              type="button"
              onClick={openCreateModal}
        ...
        <div className="flex items-center justify-between border-t border-[var(--color-border)] pt-4">
          <p className="text-xs text-[var(--color-muted-foreground)]">
            Mostrando {categories.length} de {meta.total} categorias
          </p>
