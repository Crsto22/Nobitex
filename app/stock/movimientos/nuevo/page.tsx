...
export default function NewStockMovementPage() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!branchId || reason.trim().length < 3 || !items.length) {
      showToast({
        ...
        description: "Selecciona ubicacion, productos y un motivo.",
        variant: "warning",
      });
      return;
    }
    if (
      direction === "salida" &&
      items.some((item) => item.cantidad > item.available)
    ) {
      showToast({
        title: "Stock insuficiente",
        description: "Revisa las cantidades seleccionadas.",
        variant: "warning",
      });
      return;
    }
    ...
    try {
      await stockApi.createMovement({
        ...
        items: items.map((item) => ({
          ...
          ...(direction === "entrada" && item.costoUnitario !== undefined
  ...
  return (
    <DashboardShell
      headerTitle="Nuevo movimiento"
      headerParent={{ label: "Movimientos", href: "/stock/movimientos" }}
    >
      <form
        onSubmit={submit}
        className="min-h-full space-y-4 bg-[var(--color-background)] p-4 lg:p-6"
      >
        <div className="rounded-[14px] bg-[var(--color-card)] p-4 shadow-sm">
          ...
          <p className="text-sm text-[var(--color-muted-foreground)]">
            Ajusta el stock de una tienda o almacen.
          </p>
        </div>
        ...
        <div className="grid gap-4 xl:grid-cols-[0.72fr_1.28fr]">
          <section className="space-y-5 rounded-[14px] bg-[var(--color-card)] p-4 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-2 rounded-[16px] bg-[var(--color-input-bg)] p-1">
                {(["entrada", "salida"] as StockDirection[]).map((value) => (
                  <button
                    ...
                    className={`flex h-10 items-center justify-center gap-2 rounded-[13px] text-sm font-circular-bold transition ${direction === value ? "bg-[var(--color-primary)] text-white shadow-sm" : "text-[var(--color-muted-foreground)]"}`}
                  >
                    {value === "entrada" ? (
            ...
            <Select
              ...
              options={branches.map((branch) => ({
                value: branch.id,
                label: `${branch.nombre} · ${branch.tipo === "tienda" ? "Tienda" : "Almacen"}`,
              }))}
            />
            <div>
              <label
                htmlFor="movement-reason"
                className="mb-2 block text-sm text-[#4e5671]"
              >
                Motivo
              </label>
              <textarea
                ...
                maxLength={500}
                rows={5}
                placeholder="Describe por que se realiza el movimiento"
                className="w-full resize-none rounded-[16px] bg-[var(--color-input-bg)] p-4 text-sm outline-none focus:ring-2 focus:ring-[var(--color-primary)]/20"
              />
            </div>
            <div className="rounded-[14px] bg-[var(--color-background)] p-4">
              ...
              <div className="mt-2 flex items-end justify-between">
                ...
                <p className="text-2xl font-circular-bold text-[var(--color-primary)]">
                  {totalUnits}
                </p>
              </div>
            </div>
          ...
          <section className="rounded-[14px] bg-[var(--color-card)] p-4 shadow-sm">
            <div className="mb-4">
              ...
              <p className="text-sm text-[var(--color-muted-foreground)]">
                Busca y agrega las presentaciones que deseas ajustar.
              </p>
            </div>
            <StockItemPicker
              sucursalId={branchId}
              items={items}
              onChange={setItems}
              enforceAvailable={direction === "salida"}
              showCostInput={direction === "entrada"}
            />
          </section>
        </div>
        ...
        <div className="flex justify-end gap-3">
          <button
            type="button"
            onClick={() => router.push("/stock/movimientos")}
