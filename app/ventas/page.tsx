...
function affectsIgv(tipoAfectacionIgvCodigo: string) {
  return !tipoAfectacionIgvCodigo || tipoAfectacionIgvCodigo.startsWith("1");
}
    ...
    <div className="relative">
      ...
      >
        <span className="flex min-w-0 items-center gap-2">
          ...
          <span className="font-circular-regular truncate text-sm text-[var(--color-text)]">
        ...
        <div className="absolute right-0 top-full z-20 mt-2 w-full rounded-xl bg-[var(--color-card)] p-1 shadow-lg ring-1 ring-[var(--color-border)]">
          {options.map((option) => (
            <button
              ...
              className={cn(
                "font-circular-regular flex w-full items-center justify-between rounded-lg px-3 py-2.5 text-left text-sm transition-colors",
...
export default function VentasPage() {
  ...
  const [selectedNoteType, setSelectedNoteType] = useState("nota");
  ...
  const [isLoadingCashRegister, setIsLoadingCashRegister] = useState(false);
  ...
  const requiresCashRegister = selectedBranchInfo?.modoCajaHabilitado === true;
  const cashRegisterTotals = useMemo(
    () =>
      (currentCashRegister?.totalesPorMetodoPago ?? [])
        .filter(
          (item) => item.metodoPago && Math.abs(Number(item.monto)) > 0.005,
        )
        .slice(0, 4),
    [currentCashRegister],
  );
    ...
    const timeoutId = window.setTimeout(() => {
      branchesApi
        ...
        .then((response) => {
          ...
          setSelectedBranch(
            (currentBranch) => currentBranch || defaultBranch?.id || "",
          );
        })
        .catch(() => {});
    }, 0);
    ...
    const timeoutId = window.setTimeout(() => {
      companyApi
        ...
        .then((config) => {
          ...
          const nextIgvPercent = Number(config.igvPorcentaje);
          if (Number.isFinite(nextIgvPercent) && nextIgvPercent >= 0) {
            setIgvPercent(nextIgvPercent);
          }
        })
        .catch(() => {});
    }, 0);
  ...
  useEffect(() => {
    if (!selectedBranch || !requiresCashRegister) {
      const timeoutId = window.setTimeout(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      ...
      cashRegisterApi
        ...
        .then((session) => {
          if (isMounted) {
            setCurrentCashRegister(session);
            setIsOpenCashModalOpen(!session);
          }
        })
        .catch(() => {
          if (isMounted) {
  ...
  }, [requiresCashRegister, selectedBranch]);
  ...
  }, [discountType, discountValue, rawDiscountValue, subtotal]);
  ...
  const taxSummary = useMemo<TaxSummary>(() => {
    const enabled =
      selectedNoteType === "boleta" || selectedNoteType === "factura";
    const emptySummary = {
      enabled,
      igvPercent,
      opGravadas: 0,
    ...
    const summary = cartItems.reduce(
      (current, item, index) => {
        const lineSubtotal = roundMoney(item.priceValue * item.quantity);
        const lineDiscount =
          index === cartItems.length - 1
            ? roundMoney(discountAmount - current.discountApplied)
            : subtotal > 0
              ? roundMoney((lineSubtotal * discountAmount) / subtotal)
              : 0;
        const lineTotal = roundMoney(Math.max(lineSubtotal - lineDiscount, 0));

        if (affectsIgv(item.tipoAfectacionIgvCodigo)) {
          const base = factor > 0 ? roundMoney(lineTotal / factor) : lineTotal;
          return {
            ...current,
            discountApplied: roundMoney(current.discountApplied + lineDiscount),
            opGravadas: roundMoney(current.opGravadas + base),
            igv: roundMoney(current.igv + roundMoney(lineTotal - base)),
          };
        }

        if (item.tipoAfectacionIgvCodigo.startsWith("2")) {
          return {
            ...current,
            discountApplied: roundMoney(current.discountApplied + lineDiscount),
            opExoneradas: roundMoney(current.opExoneradas + lineTotal),
          };
        }
        ...
        return {
          ...current,
          discountApplied: roundMoney(current.discountApplied + lineDiscount),
          opInafectas: roundMoney(current.opInafectas + lineTotal),
        };
      },
      {
        discountApplied: 0,
    ...
    return {
      enabled,
      igvPercent,
      opGravadas: summary.opGravadas,
      opExoneradas: summary.opExoneradas,
      opInafectas: summary.opInafectas,
      igv: summary.igv,
      total,
    };
  }, [
    ...
    setCartItems((currentItems) => {
      ...
      return [
        ...currentItems,
        {
          ...
          tipoAfectacionIgvCodigo: product.tipoAfectacionIgv.codigo,
          product,
        },
      ];
    });
  ...
  const handleSaleSuccess = () => {
    ...
    refreshProducts();
    if (requiresCashRegister && selectedBranch) {
      cashRegisterApi
        .current(selectedBranch)
        .then((session) => setCurrentCashRegister(session))
        .catch(() => {});
    }
  };
    ...
    <DashboardShell headerTitle={cashHeader}>
        ...
        <aside className="flex h-full min-h-0 w-full shrink-0 flex-col lg:w-[360px]">
              ...
              {user?.sucursalId ? (
                <div className="flex h-11 items-center gap-2 rounded-[14px] bg-[var(--color-input-bg)] px-3 text-xs font-circular-bold text-[var(--color-text)]">
                  ...
                  <span className="truncate">
                    {selectedBranchInfo?.nombre ?? "Sucursal asignada"}
                  </span>
                </div>
              ) : (
                <DropdownButton
          ...
          <div className="mt-3 flex flex-col gap-3 px-2">
            <div ref={clientDropdownRef} className="relative">
              ...
              {isClientDropdownOpen && (
                <div className="absolute left-0 top-full z-50 mt-2 w-full overflow-hidden rounded-xl bg-[var(--color-card)] shadow-xl ring-1 ring-[var(--color-border)] animate-in fade-in zoom-in-95 duration-200">
                  <div className="relative border-b border-[var(--color-border)] bg-[var(--color-input-bg)]/50 px-3 py-2">
                    <MagnifyingGlassIcon
                      size={16}
                      className="pointer-events-none absolute left-6 top-1/2 -translate-y-1/2 text-[var(--color-muted-foreground)]"
                    />
                    <input
                      ref={clientSearchRef}
                      type="text"
                  ...
                  <div className="max-h-60 overflow-y-auto p-1">
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <span className="font-circular-bold block truncate text-sm">
                          Crear cliente
                        </span>
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <span className="font-circular-regular block truncate text-sm">
                          Cliente generico
                        </span>
                        <span
                          className={cn(
                ...
                {cartItems.map((item) => (
                  ...
                  >
                    <div className="relative h-16 w-16">
                      ...
                      {item.tipo === "variantes" ? (
                        <div
                          className="absolute top-0 left-0 flex h-5 w-5 items-center justify-center rounded-full text-[8px] font-circular-bold text-white shadow-sm"
          ...
          <button
            type="button"
            onClick={() => {
              if (requiresCashRegister && !currentCashRegister) {
                setIsOpenCashModalOpen(true);
                return;
              }
      ...
      {isChargeModalOpen ? (
        <ChargeModal
          ...
          cartItems={cartItems}
          subtotal={subtotal}
          discountType={hasDiscountApplied ? discountType : null}
      ...
      <OpenCashRegisterModal
        ...
        sucursalId={selectedBranch}
        sucursalNombre={selectedBranchInfo?.nombre}
        onSuccess={(session) => setCurrentCashRegister(session)}
      />
      ...
      <CloseCashRegisterModal
        ...
        onSuccess={() => setCurrentCashRegister(null)}
      />
